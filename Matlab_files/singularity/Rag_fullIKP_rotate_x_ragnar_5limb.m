...
function [sol, sol2] = Rag_fullIKP_rotate_x_ragnar_5limb(pose, h_all)
...
r_a = [0.08; 0; 0.024];
    ...
    % end 
    C = P + h_d * Rx(phi) * u_j(:,1) + r_a;
    C = P + Rx(phi)*h_all(:,1) + r_a;
    ...
    SOL = [2*atan((-I + sqrt(sqrt_Term))/(K - J));
        2*atan((-I - sqrt(sqrt_Term))/(K - J))];
    
    %%%%%%%%%%%%%%%%%%%%%%%% solving for the passive joints
    R1 = Rz(alpha) * Ry(beta) * Rz(SOL(2));
    A1 = [a; b; 0];
    S1_1 = R1 * (l*ii);
    B1 = A1+S1_1;
