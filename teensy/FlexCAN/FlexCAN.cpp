...
int FlexCAN::read(CAN_message_t &msg)
{
  ...
  /* If you have set timout to 0 in the pkg you want to receive in
   * then you will abort if there is no pkg alrdy in the controller for you
   * or wait a max of timesout msecs before you abort if no kg has been received
   * NBNB timing is in msec(millisec)
  */
  while( !available() ) {
  ...
  // write 0 in the 4 bytes taht was not in the pkg receievd due to short pkg-
