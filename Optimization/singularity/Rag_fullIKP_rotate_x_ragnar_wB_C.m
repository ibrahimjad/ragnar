...
function [sol, sol2,Bret,Cret,Aret] = Rag_fullIKP_rotate_x_ragnar_wB_C(parameter, pose, h_all)
% create function handles for rotations 
Rx = @(angle) ([1 0 0; 0, cos(angle) -sin(angle); 0, sin(angle) cos(angle)]); 
dRx = @(angle) ([0 0 0; 0, -sin(angle) -cos(angle); 0, cos(angle) -sin(angle)]); 
